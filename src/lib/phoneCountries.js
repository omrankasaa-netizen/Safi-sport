/**
 * phoneCountries — compact international dial-code list + helpers for the
 * storefront phone inputs (Checkout, TrackOrder, CustomDesign).
 *
 * The full ITU E.164 country-code set (~240 entries) with English names.
 * Display order: Lebanon first (pinned — this is a Lebanese brand and the
 * default audience), then the other Arab/Gulf countries, then everything
 * else alphabetical.
 *
 * This is a compact internal list, not a phone-metadata library: shared
 * codes (e.g. +1 for the whole NANP, +7 for RU/KZ) resolve to whichever
 * entry carries the longest matching dial code, which is good enough for
 * pre-filling a country picker. Validation below is a sanity check, not
 * authoritative numbering-plan data.
 */
...
/** ISO-3166 alpha-2 -> regional-indicator flag emoji (e.g. "LB" -> flag). */
export function flagEmoji(iso) {
  return String(iso)
    .toUpperCase()
    .replace(/[A-Z]/g, (ch) => String.fromCodePoint(0x1f1a5 + ch.charCodeAt(0)));
}

/** Select label like "LB +961 Lebanon" (flag prefix added by the picker). */
export function countryOptionLabel(c) {
  return `${flagEmoji(c.iso)} +${c.dial} ${c.name}`;
}

// Longest-dial-code-first ordering for splitInternational — a number like
// +1268... must resolve to Antigua (+1268), not Canada/USA (+1).
const BY_DIAL_LENGTH_DESC = [...COUNTRIES].sort(
  (a, b) => b.dial.length - a.dial.length,
);

/**
 * Detect a full international number in pasted/typed text.
 * Given "+971 50 123 4567" or "00971501234567", returns
 * { iso: "AE", national: "501234567" }; otherwise null (bare national
 * numbers return null — we never guess a country).
 */
...
/** Normalize dial + national part to E.164: strip non-digits and the
 *  national trunk zero (e.g. ("961", "03 701 234") -> "+9613701234"). */
...
// Lebanon sanity check — NOT an authoritative numbering plan. Mobiles start
// 3/70/71/76/78/79/81, landline area codes start 1/4/5/6/7/8/9; the union
// reduces to "first digit 1 or 3-9" on a 7-8 digit national number.
const LB_MOBILE_PREFIX = /^(3|70|71|76|78|79|81)/;
const LB_LANDLINE_PREFIX = /^[1-9]/;

function isPlausibleLebaneseNational(nationalDigits) {
  if (!/^\d{7,8}$/.test(nationalDigits)) return false;
  return LB_MOBILE_PREFIX.test(nationalDigits) || LB_LANDLINE_PREFIX.test(nationalDigits);
}

/**
 * Validate the picker state. Returns null when OK, otherwise an error key
 * ("invalid") the caller maps to a bilingual message.
 */
export function validatePhone(iso, dial, national, _lang) {
  const nationalDigits = String(national ?? "")
    .replace(/\D/g, "")
    .replace(/^0+/, "");
  if (nationalDigits.length < 4 || nationalDigits.length > 13) return "invalid";
  const e164 = toE164(dial, national);
  if (!E164_REGEX.test(e164)) return "invalid";
  if (iso === "LB" && !isPlausibleLebaneseNational(nationalDigits)) return "invalid";
  return null;
}
